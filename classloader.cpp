...
/**===-------------------  BootStrap ClassLoader ------ important----------------===*/
                                       ...
                                       bool, InstanceKlass *, ObjArrayOop *)
{
    ...
    } else if (classname.find_first_of(L"[")==0) {
        if (system_classmap.find(target) != system_classmap.end()) {	// has been loaded
            return system_classmap[target];
        } else {	// load
            int pos = 0;
...
/**===-------------------  My ClassLoader -------------------===*/
Klass *AutomanClassLoader::loadClass(const wstring & classname, ByteStream *byte_buf, MirrorOop *loader_mirror,
                                bool is_anonymous, InstanceKlass *hostklass, ObjArrayOop *cp_patch)
{
    ...
    if (is_anonymous) {
        ...
        return result;
        //todo: equals to  starts with
    } else if (!classname.find_first_of( L"[")==0) {	// not '[[Lcom/zxl/Haha'.
        // TODO: 可以加上 classpath。
        wstring target = classname + L".class";
...
Klass *AutomanClassLoader::find_in_classmap(const wstring & classname)
{
    LockGuard lg(this->lock);
...
void AutomanClassLoader::cleanup()
{
    LockGuard lg(this->lock);
