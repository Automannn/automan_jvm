...
void InstanceKlass::parse_interfaces(ClassFile *cf, ClassLoader *loader)	// interface should also be made by the InstanceKlass!!
{
    for(int i = 0; i < cf->interfaces_count; i ++) {
        ...
        if (loader == nullptr) {
            _interface = ((InstanceKlass *)BootStrapClassLoader::get_bootstrap().loadClass(interface_name));
        } else {
            _interface = ((InstanceKlass *)loader->loadClass(interface_name));
            assert(_interface != nullptr);
        }
        assert(_interface != nullptr);
        this->interfaces.insert(make_pair(interface_name, _interface));
    }
#ifdef KLASS_DEBUG
...
void InstanceKlass::parse_method(ClassFile *cf)
{
    ...
    for(int i = 0; i < cf->methods_count; i ++) {
        ...
        MethodPool::put(method);		// put it into global area
...
//				std::wcout << "initialize ConstantValue_attribute !" << std::endl;		// delete
                switch (_pair.first) {
                    case CONSTANT_Long:{
                        this->set_static_field_value(iter.first, new LongOop(*((long*)(_pair.second))));
                        break;
                    }
                    case CONSTANT_Float:{
                        this->set_static_field_value(iter.first, new FloatOop(*((float*)(_pair.second))));
                        break;
                    }
                    case CONSTANT_Double:{
                        this->set_static_field_value(iter.first, new DoubleOop(*((double*)(_pair.second))));
                        break;
                    }
                    case CONSTANT_Integer:{
                        this->set_static_field_value(iter.first, new IntOop(*((int*)(_pair.second))));
                        break;
                    }
                    case CONSTANT_String:{
                        this->set_static_field_value(iter.first, (Oop *)(_pair.second));
                        break;
                    }
                    default:{
...
wstring InstanceKlass::parse_signature()
{
    ...
    wstring* signature = (wstring*)(_pair.second);
    assert(*signature != L"");
    return *signature;
}
