...
list<Method *> & MethodPool::method_pool() {
    static list<Method *> method_pool;
    return method_pool;
}

void MethodPool::put(Method *method) {
    LockGuard lg(method_pool_lock());
    method_pool().push_back(method);
}

void MethodPool::cleanup() {
    LockGuard lg(method_pool_lock());
...
vector<MirrorOop *> Method::parse_argument_list(const wstring & descriptor)
{
    ...
    for (int i = 0; i < args.size(); i ++) {
        if (args[i].size() == 1) {		// primitive type
            ...
            Klass *klass = AutomanClassLoader::get_loader().loadClass(args[i].substr(1, args[i].size() - 2));
            assert(klass != nullptr);
            v.push_back(klass->get_mirror());
        } else {		// ArrayType
            assert(args[i][0] == L'[');
            Klass *klass = AutomanClassLoader::get_loader().loadClass(args[i]);
            assert(klass != nullptr);
            v.push_back(klass->get_mirror());
        }
    }
...
//		}
        Klass *klass =AutomanClassLoader::get_loader().loadClass(return_type.substr(1, return_type.size() - 2));
        assert(klass != nullptr);
...
//		}
        Klass *klass = AutomanClassLoader::get_loader().loadClass(return_type);
        assert(klass != nullptr);
        return klass->get_mirror();
    }
}
...
int Method::where_is_catch(int cur_pc, InstanceKlass *cur_excp)
{
	...
	sync_wcout{} << "===---------------- [End Finding Catch/Finally Block [V]] -----------------===" << std::endl;
                ...
                auto catch_klass = ((InstanceKlass *)((Klass *)(_pair.second)));
