...
void GC::cancel_gc_thread()
{
    while(true) {
        ...
        if (GC::gc()) {
            ...
            TerminateThread(automan_jvm::gc_thread(),0);
            //todo: 注意，gc线程的退出是由 jvm外的线程完成的
            WaitForSingleObject(automan_jvm::gc_thread(),INFINITE);
            break;
        }
    }
}