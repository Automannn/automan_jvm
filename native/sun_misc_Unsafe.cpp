...
void JVM_GetByte(list<Oop *> & _stack){
    //todo: 由于 unsafe 保存的内容失真，导致nio.Bits初始化报错；
    InstanceOop *_this = (InstanceOop *)_stack.front();
    _stack.pop_front();
    long addr = ((LongOop *)_stack.front())->value;	_stack.pop_front();
    uint8_t val = *((uint8_t *)addr);
    //todo: 这里不知道为什么，java.nio.Bits 存储的内容失真了，导致无法判断大端还是小端,这里手动设置为小端，极不安全，后期有精力必须修改
    if(_this->get_klass()->get_name()==L"sun/misc/Unsafe"&&val==12){
...
//	byte_buf.print(',', true);
    ...
    auto anonymous_klass = AutomanClassLoader::get_loader().loadClass(klass_name, &byte_buf, host_klass_loader,
...
void JVM_EnsureClassInitialized(list<Oop *> & _stack){
    ...
    BytecodeEngine::initial_client(real_klass, *thread);
}
...
void JVM_DefineClass(list<Oop *> & _stack){
    ...
    Klass *klass;
    if (loader != nullptr) {
        klass = AutomanClassLoader::get_loader().loadClass(klass_name, &byte_buf, ((InstanceKlass *)loader->get_klass())->get_java_loader());
    } else {
        klass = AutomanClassLoader::get_loader().loadClass(klass_name, &byte_buf, nullptr);
    }
