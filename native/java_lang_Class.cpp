    ...
    if (loader != nullptr) {
        ...
        if (initialize) {
            if (klass->get_type() == ClassType::InstanceClass)	// not an ArrayKlass
                BytecodeEngine::initial_client(((InstanceKlass *)klass), thread);
        }
...
void JVM_GetDeclaringClass(list<Oop *> & _stack){
    ...
    InnerClasses_attribute* inner_class_attr = static_cast<InnerClasses_attribute *>(klass->get_inner_class());
    ...
    for (int i = 0; i < inner_class_attr->number_of_classes; i ++) {
        ...
        if (ioff != 0) {
            auto target_inner_klass =(Klass *)((*rt_pool)[ioff-1].second);
            if (target_inner_klass == klass) {		// get the inner is `this`. then find the outer.
                if (ooff == 0) {
                    _stack.push_back(nullptr);
                    return;
                } else {
                    auto target_outer_klass = (Klass *)((*rt_pool)[ooff-1].second);
...
void JVM_GetEnclosingMethodInfo(list<Oop *> & _stack){
    ...
    auto klass = ((InstanceKlass *)_this->get_mirrored_who());
    auto enclosing_method_attr = (EnclosingMethod_attribute*)klass->get_enclosing_method();
    ...
    auto target_klass = (Klass *)((*rt_pool)[target_klass_index-1].second);
    ...
    pair<int, int>* _pair = (pair<int, int>*)((*rt_pool)[target_name_and_type_index-1].second);
    // 2.3. get the String: name
    auto name = java_lang_string::intern(*(wstring*)((*rt_pool)[_pair->first-1].second));
    // 2.5. put it in.
    (*obj_arr)[1] = name;
    // 2.6. get the String: descriptor
    auto descriptor = java_lang_string::intern(*(wstring*)((*rt_pool)[_pair->second-1].second));
    // 2.8. put it in.
